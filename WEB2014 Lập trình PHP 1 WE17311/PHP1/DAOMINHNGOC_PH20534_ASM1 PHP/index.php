<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ASIGNMENT GD1</title>
    <link rel="stylesheet" href="css/main.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@100&display=swap" rel="stylesheet">

</head>

<body>

    <!-- Page nav -->
    <nav class="">
        <!-- Container -->
        <div class="container">
            <div class="nav-main">
                <img src="img/logo.jpg" class=" ml-6 my-4" width="200" height="50" alt="">
                <!-- nav -->
                <ul class="header-nav">
                    <li><a href="index.php" style="color: red;">Trang chủ</a></li>
                    <li><a href="gioithieu.php">Giới thiệu</a></li>
                    <li><a href="lienhe.php">Liên hệ</a></li>
                    <li><a href="sanpham.php">Sản phẩm</a>
                        <ul class="sub-menu">
                            <li><a href="">Quần áo mùa đông</a></li>
                            <li><a href="">Quần áo nam</a></li>
                            <li><a href="">Quần áo nữ</a></li>
                            <li><a href="">Quần áo mùa hè</a></li>
                        </ul>
                    </li>
                    <li><a href="tintuc.html">Tin tức</a></li>
                </ul>
            </div>


            <form action="" class="search">
                <input class="ip-search text-20" type="text" placeholder="search" required>
                <button class="btn-search" type="submit">Tìm kiếm</button>
            </form>


            <div class="frm-dangnhap">
                <a href="">Đăng ký</a>
                <span>|</span>
                <a href="">Đăng nhập</a>
            </div>
        </div>
    </nav>
    <!-- end page nav -->

    <section class="banner">
        <div>
            <img src="img/banner.jpg" width="1100" height="400" alt="">
        </div>
    </section>

    <!-- PRODUCT -->
    <section class="product">
        <div class="title">
            <h3 class="">Các sản phẩm hiện có</h3>
            <h4>
                <hr>
            </h4>
        </div>
        <div class="row">
            <div class="col">
                <a href=""><img src="img/áo1.jpg" width="270" alt=""></a>
                <h1><a href="">Áo Thun tay lỡ hot trend 2022</a></h1>
                <span class="price">300.000 đ</span>
                <span class="price-m">299.000 đ</span>
            </div>
            <div class="col">
                <a href=""><img src="img/áo2.jpg" width="270" height="275" alt=""></a>
                <h1><a href="">Áo phông nữ dáng dấu quần hoạ tiết gấu</a></h1>
                <span class="price">300.000 đ</span>
                <span class="price-m">299.000 đ</span>
            </div>
            <div class="col">
                <a href=""><img src="img/áo3.webp" width="270" height="270" alt=""></a>
                <h1><a href="">Mẫu áo thun local brand nữ màu xanh </a></h1>
                <span class="price">300.000 đ</span>
                <span class="price-m">299.000 đ</span>
            </div>
            <div class="col">
                <a href=""><img src="img/áo4.jpg" width="270" height="270" alt=""></a>
                <h1><a href="">[áo loại 1] áo kết nơ hót trend nhất hiện nay</a></h1>
                <span class="price">300.000 đ</span>
                <span class="price-m">299.000 đ</span>
            </div>
            <div class="col">
                <a href=""><img src="img/áo5.jpg" width="270" height="270" alt=""></a>
                <h1><a href="">Kiểu phối đồ nữ phong cách hàn quốc</a></h1>
                <span class="price">300.000 đ</span>
                <span class="price-m">299.000 đ</span>
            </div>
            <div class="col">
                <a href=""><img src="img/áo6.jpg" width="270" height="270" alt=""></a>
                <h1><a href="">Áo khoác bomber 2 lớp </a></h1>
                <span class="price">300.000 đ</span>
                <span class="price-m">299.000 đ</span>
            </div>
            <div class="col">
                <a href=""><img src="img/áo9.jpg" width="270" height="270" alt=""></a>
                <h1><a href="">Kiểu áo sơ mi</a></h1>
                <span class="price">300.000 đ</span>
                <span class="price-m">299.000 đ</span>
            </div>
            <div class="col">
                <a href=""><img src="img/áo8.webp" width="270" height="270" alt=""></a>
                <h1><a href="">Kiểu áo hot trend</a></h1>
                <span class="price">300.000 đ</span>
                <span class="price-m">299.000 đ</span>
            </div>
        </div>
    </section>




    <footer>
        <section class="footer">
            <div class="row-footer">
                <div class="col-footer">
                    <h3>HỖ TRỢ KHÁCH HÀNG</h3>
                    <p>
                        <a href=""> Các câu hỏi thường gặp </a> <br>
                        <a href="">Liên hệ chúng tôi</a> <br>
                        <a href="">Hệ thống cửa hàng</a> <br>
                        <a href="">Tra cứu đơn hàng</a> <br>
                        <a href="">Hướng dẫn đo kích thước</a> <br>
                        <a href="">Chính sách đổi hàng</a> <br>
                        <a href="">Chính sách giao hàng</a> <br>
                        <a href="">Chính sách khách hàng thân thiết Tam Sơn</a> <br>
                    </p>
                </div>
                <div class="col-footer">
                    <h3>BỘ SƯU TẬP NỮ</h3>
                    <p>
                        <a href="">Bộ sưu tập Xuân/Hè</a> <br>
                        <a href="">Đầm</a><br>
                        <a href="">Áo sơmi & Áo kiểu</a><br>
                        <a href=""> Quần dài</a><br>
                        <a href=""> Ví da</a><br>
                        <a href="">Giày dép</a><br>
                    </p>
                </div>
                <div class="col-footer">
                    <h3>BỘ SƯU TẬP NAM</h3>
                    <p>
                        <a href="">Bộ sưu tập Xuân/Hè</a><br>
                        <a href="">Áo sơ mi</a><br>
                        <a href=""> Áo thun & Áo Polo</a><br>
                        <a href="">Quần dài & Quần short</a><br>
                        <a href=""> Túi xách</a><br>
                        <a href="">Giày dép</a><br>
                    </p>
                </div>
                <div class="col-footer">
                    <h3>THÔNG BÁO PHÁP LÝ</h3>
                    <p>
                        <a href="">Thông tin công ty</a><br>
                        <a href="">Điều khoản sử dụng</a><br>
                        <a href="">Chính sách bảo mật</a><br>
                        <a href="">Chính sách Cookie</a><br>
                        <a href=""> Quản lý Cookie của tôi</a><br>
                    </p>
                </div>
            </div>
        </section>
    </footer>
</body>

</html>